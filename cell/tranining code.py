...
def preprocess(imgs):
    imgs_p = np.ndarray((imgs.shape[0], img_rows, img_cols), dtype=np.uint8)        #(batch,width,height)
    for i in range(imgs.shape[0]):
        imgs_p[i] = resize(imgs[i], (img_cols, img_rows), preserve_range=True)

    imgs_p = imgs_p[..., np.newaxis]    #(batch,width,height,channel)
    return imgs_p
...
def train_and_predict():
    ...
    t=model.evaluate(imgs_test, imgs_mask_test)
    print(t)

    imgs_mask_predict = model.predict(imgs_test, verbose=1)
    np.save('imgs_mask_pred.npy', imgs_mask_predict)
